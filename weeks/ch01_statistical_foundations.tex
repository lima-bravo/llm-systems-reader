...
Shannon estimated the \emph{entropy} of English at roughly one bit per
...
Chapter~\ref{chap:transformer}), and the weights $\theta$ are learned by fitting this
conditional to empirical text (Chapter~\ref{chap:pretraining}). Everything else---the
...
positions---a phenomenon we revisit in Chapter~\ref{chap:tokenization}.
...
\textbf{Used later:} Chapter~\ref{chap:optimization} optimises (\ref{eq:mle}); Chapter~\ref{chap:transformer}
parameterises $q_\theta$ via attention; Chapter~\ref{chap:alignment} replaces $\hat p$ with
...
in Chapter~\ref{chap:alignment} where reverse-KL and forward-KL yield qualitatively different
...
is revisited in Chapter~\ref{chap:tokenization}.
...
still being useless at answering a question. Chapter~\ref{chap:systems} confronts task-level
evaluation directly.
...
\textbf{Used later:} Chapter~\ref{chap:pretraining} uses (\ref{eq:perplexity}) as a headline
metric; Chapter~\ref{chap:alignment} uses (\ref{eq:kl-def}) as the KL penalty in DPO.
\end{provedassumed}
...
\textbf{Used later:} Chapter~\ref{chap:pretraining} revisits temperature as a decoding knob;
Chapter~\ref{chap:alignment} tracks calibration drift after alignment; Chapter~\ref{chap:systems} uses
...
decoding strategy. Chapter~\ref{chap:pretraining} treats decoding more formally; here the
...
lossy (Chapter~\ref{chap:tokenization}), semantically equivalent inputs can produce very
...
\Emp\ Post-training techniques (instruction tuning, RLHF; Chapter~\ref{chap:alignment})
...
and tool-using systems (Chapter~\ref{chap:systems}).
...
\begin{itemize}
  ...
  \item \citet{vapnik1998statistical} --- \emph{Statistical Learning Theory}; classical PAC framework for generalization, useful background for Chapter~\ref{chap:optimization}.
