...
random either. It is shaped by everything you have ever read, and
...
single sentence. Everything in this book is the elaboration.
...
proper scoring rule. It is the information-theoretic distance
between two distributions, plus a constant the model cannot
control. We will see all three of these views in the pages that
...
that the model has no native concept of confidence in the sense a
human reader would mean. It has a probability distribution over
...
\begin{itemize}
  \item[A1.1] Token sequences are realisations of a stochastic process; the
  set of (context, token) pairs used for MLE is treated as i.i.d.\ under
  the empirical measure $\hat{p}$.
...
Week~4), and the weights $\theta$ are learned by fitting this
conditional to empirical text (Week~5). Everything else---the
...
Using A1.1 (i.i.d.\ sequences) and the chain rule (\ref{eq:chain}):
\begin{align}
P_\theta(\mathcal{D})
  &= \prod_{i=1}^{N} P_\theta(x^{(i)})
  && \text{(A1.1: i.i.d.\ sequences)} \label{eq:mle-1} \\
...
$\hat p$. This is a general fact about MLE under exponential-family
parameterisations and is not specific to language modelling.
...
\subsubsection{Worked Example: Softmax and LSE}
...
positions---a phenomenon we revisit in Week~3.
...
\textbf{Assumed here:} A1.1 (i.i.d.\ pairs), A1.2 (strictly positive
...
\textbf{Used later:} Week~2 optimises (\ref{eq:mle}); Week~4
parameterises $q_\theta$ via attention; Week~8 replaces $\hat p$ with
...
in Week~8 where reverse-KL and forward-KL yield qualitatively different
...
\Cons\ Perplexity has an intuitive reading: it is the ``effective
vocabulary size'' the model is uncertain over at each step. A
perplexity of $1$ corresponds to a perfectly deterministic predictor; a
perplexity of $|V|$ corresponds to the uniform distribution. The
...
still being useless at answering a question. Week~9 confronts task-level
evaluation directly.
...
\textbf{Used later:} Week~5 uses (\ref{eq:perplexity}) as a headline
metric; Week~8 uses (\ref{eq:kl-def}) as the KL penalty in DPO.
\end{provedassumed}
...
\Def\ The \emph{Expected Calibration Error} (ECE) is the
$L_1$-weighted calibration gap across bins:
\begin{equation}
\mathrm{ECE}(q; \mathcal{D}_{\mathrm{cal}})
  := \E_{p^\star \sim \mathrm{dist}(\hat q)}
       \big[\, \big| \Pr[\text{correct} \mid \hat q = p^\star] - p^\star \big| \,\big].
...
and plug-in estimators for the conditional expectations.
\end{derivation}
...
$\mathrm{conf} = 0.30$, $\mathrm{acc} = 0.50$. In bin~2,
$\mathrm{conf} = 0.90$, $\mathrm{acc} = 1.10$ is impossible; instead use
$\mathrm{conf} = 0.70$, $\mathrm{acc} = 0.90$ (\emph{under}confident by
$0.20$). Then
\[
...
\textbf{Used later:} Week~5 revisits temperature as a decoding knob;
Week~8 tracks calibration drift after alignment; Week~9 uses
...
decoding strategy. Week~5 treats decoding more formally; here the
...
lossy (Week~3), semantically equivalent inputs can produce very
...
\Emp\ Post-training techniques (instruction tuning, RLHF; Week~8)
...
\subsection*{Derivation Ledger (Ch. 1)}
...
\begin{itemize}
  ...
  \item \textbf{MLE $\to$ token-normalised NLL $\to$ cross-entropy.}\
        ...
        assumption \textbf{(A1.1)} i.i.d.\ sequences applied at line
  ...
  \item \textbf{ECE (finite-bin estimator).}\
        $\mathrm{ECE} = \sum_{b=1}^{B} (|B_b|/N)\,|\bar a_b - \bar c_b|$
