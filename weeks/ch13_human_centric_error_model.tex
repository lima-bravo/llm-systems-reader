...
The five axes map single-output failures. Four additional failure
classes exist alongside them that the single-output frame cannot
reach: normative, adversarial, drift, and complacency. The chapter
maps all nine before demonstrating the ladder in practice.
...
\begin{itemize}
  ...
  \item State the limits of the model: where the axes overlap,
        ...
        and which four failure classes escape the framework
        entirely (normative, adversarial, drift, complacency).
...
\begin{itemize}
  ...
  \item Limits of the framework: four failure classes that escape
        the five axes --- normative, adversarial, drift, and
        complacency.
\end{itemize}
...
system-level interventions (Chapter~\ref{chap:systems}, Chapter~\ref{chap:agents}) layered on top
...
instances of the same idea. Function-calling APIs (Chapter~\ref{chap:agents})
...
trained on cross-entropy (Chapter~\ref{chap:stat-foundations}) optimizes for the
...
(Chapter~\ref{chap:agents}) extend the same pattern by adding explicit
...
list above. These are, in the language of Chapter~\ref{chap:pretraining}, ways of
...
Few-shot prompting (Chapter~\ref{chap:pretraining}) is, among other things, an attempt
to encode situational expectations through examples. System prompts
(Chapter~\ref{chap:agents}) are an attempt to fix the situation up front.
Alignment training (Chapter~\ref{chap:alignment}) shapes the model's defaults along
...
The remedy at the system level is retrieval (Chapter~\ref{chap:systems}). A
...
Grounding failures are compounded by a calibration problem. The
model's expressed confidence --- the surface certainty of its
prose --- is not a reliable signal of actual reliability. A
hallucinated statistic arrives in the same confident register as
a verified fact. Token-level probabilities correlate with accuracy
above chance, and the internal signal is non-zero, but the
correlation is not strong enough to substitute for external
verification and is inaccessible to users who read prose rather
than logits. The practical consequence is that expressed
confidence cannot be used to triage which claims need checking.
The discipline of treating every factual claim as a hypothesis
applies uniformly, not selectively to claims that sound uncertain.
...
When grounding failures appear consistently in a specific domain,
register, or procedural context, the root cause may be
distributional: the model's fine-tuning distribution is
misaligned with the deployment distribution (Chapter~\ref{chap:alignment}). Retrieval
and tool use reduce individual grounding errors but do not address
the underlying mismatch. The systemic remedy is targeted evaluation
on representative examples from the actual deployment distribution,
...
through retrieval (Chapter~\ref{chap:systems}), through tool use, through careful
...
everything diagnosable when a specific output is in front of you.
Six additional failure classes exist alongside them; they escape
the single-output frame entirely and require different detection
and remediation. The diagnostic ladder in the next section
addresses only the five axes. Knowing what lies beyond them before
walking makes the ladder's scope explicit and its limits visible.
...
hold. These failures live in Chapter~\ref{chap:alignment}'s territory rather than
...
retrieval source, or a manipulated tool. The most practically
important variant for systems in production is \emph{indirect
prompt injection}: malicious instructions embedded not in the
user's prompt but in content the model is asked to process ---
retrieved documents, tool outputs, web pages fetched by an agent
(Chapter~\ref{chap:agents}). A retrieval pipeline (Chapter~\ref{chap:systems}) that fetches
...
document to the system designer may be interpreted as a prompt
extension by the model. These failures cross the trust boundaries
of Chapters~10 and~12 and are addressed by defenses in depth ---
input sanitization, output schema enforcement, privilege
separation between retrieval and execution --- rather than by
diagnostic ladders.
...
governance (Chapter~\ref{chap:governance}).
...
throughput. Chapter~\ref{chap:systems} (\S\ref{sec:verification-cost-erosion})
...
provides: alignment and value work for normative failures;
security engineering and defense in depth (Chapters~10,~12) for
adversarial failures; monitoring and governance cycles
(Chapter~\ref{chap:governance}) for drift; deliberate review-quality engineering for
complacency. The ladder below addresses the five-axis failures
--- the class most commonly encountered in practice and most
directly remedied by prompt, system design, and process changes.
Hold the axes loosely: they are not orthogonal, failures often
span multiple, and the ordering is a heuristic, not a theorem.

\section{The Diagnostic Ladder}

With the full failure landscape in view, the ladder's scope is
clear: it is a systematic procedure for the five-axis failures.
...
\begin{enumerate}
  \item \emph{Structure}. Does the output parse, compile, validate
        ...
        the remedy lives in Chapter~\ref{chap:pretraining} (decoding) or Chapter~\ref{chap:agents}
...
For tasks that chain multiple inference steps --- a multi-step
reasoning trace, an agentic pipeline, a code refactor threading
through interdependent modules --- apply the ladder at each
...
correct: accuracy $p$ on each step gives approximately $p^k$
reliability on a $k$-step chain under step independence, and
early errors constrain downstream reasoning. The architectural
remedy is checkpointing and intermediate verification
(Chapters~11,~12), not per-step prompt improvement.
...
--- is a grounding failure produced by the mechanism of Chapter~\ref{chap:stat-foundations}:
...
\textbf{Structure} is the territory of decoding (Chapter~\ref{chap:pretraining}),
constrained sampling, and the structured-output contracts of tool
use (Chapter~\ref{chap:agents}). The mathematical objects involved are the
...
(Chapter~\ref{chap:stat-foundations}), embeddings (Chapter~\ref{chap:tokenization}), and the inference-compute
trade-offs of self-checking and chain-of-thought (Chapter~\ref{chap:pretraining}). The
...
(Chapter~\ref{chap:pretraining}), prompt construction (Chapter~\ref{chap:systems}), system prompts
(Chapter~\ref{chap:agents}), and the tone-and-style components of alignment
(Chapter~\ref{chap:alignment}). The mathematical objects involved are the
preference-model log-likelihood, the few-shot prompt format, and
the RLHF KL penalty that keeps the policy from over-tuning.

\textbf{Grounding} is the territory of retrieval (Chapter~\ref{chap:systems}), tool
use, the symbol-grounding literature \citep{harnad1990symbol,
bender2020climbing}, and the memorization-versus-hallucination
distinction of Chapter~\ref{chap:pretraining}. The mathematical objects involved are
BM25, the dense-retrieval inner product, the ReAct loop, and the
verification predicates of Chapter~\ref{chap:governance}'s hazard analysis.

\textbf{Environment} is the territory of system design rather than
model design. Long-context architectures (Chapter~\ref{chap:transformer}) extend how
much environment can be imported. Agent loops (Chapter~\ref{chap:agents})
accumulate environment over time. Documentation, logging, and
incident reporting (Chapter~\ref{chap:governance}) make the environment the
...
\begin{itemize}
  \item \textbf{Chapter~\ref{chap:stat-foundations} (Statistical Foundations).} The
  ...
  \item \textbf{Chapter~\ref{chap:tokenization} (Tokenization and Embeddings).} The
        embedding space is the substrate on which meaning is
        encoded; the structure axis sits on top of the
        tokenization pipeline.
  \item \textbf{Chapter~\ref{chap:transformer} (Transformer Architecture).} Context
  ...
  \item \textbf{Chapter~\ref{chap:pretraining} (Pretraining and In-Context Learning).}
  ...
  \item \textbf{Chapter~\ref{chap:alignment} (Alignment).} Alignment shapes the
  ...
  \item \textbf{Chapter~\ref{chap:systems} (RAG, Tools, Evaluation).} Retrieval is
  ...
  \item \textbf{Chapter~\ref{chap:governance} (Governance).} The diagnostic ladder
  ...
  \item \textbf{Chapter~\ref{chap:agents} (APIs, Agents, MCP).} Agents accumulate
...
why the alignment failure modes of Chapter~\ref{chap:alignment} are not engineering
