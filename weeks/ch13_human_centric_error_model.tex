...
The five axes map single-output failures. Six additional failure
classes exist alongside them that the single-output frame cannot
reach: normative, adversarial, drift, complacency, compositional
degradation, and distributional mismatch. The chapter maps all
eleven before demonstrating the ladder in practice.
...
\begin{itemize}
  ...
  \item State the limits of the model: where the axes overlap,
        ...
        and which six failure classes escape the framework
        entirely (normative, adversarial, drift, complacency,
        compositional degradation, distributional mismatch).
...
\begin{itemize}
  ...
  \item Limits of the framework: six failure classes that escape
        the five axes --- normative, adversarial, drift,
        complacency, compositional degradation, and
...
(Chapter~12). A retrieval pipeline (Chapter~10) that fetches
...
The fifth is the \emph{compositional degradation} failure. When a
task requires chaining multiple inferences --- a multi-step
reasoning trace, an agentic pipeline that calls several tools in
sequence, a code refactor threading through five interdependent
modules --- errors compound across steps. A model that achieves
accuracy $p$ on each step achieves approximately $p^k$ accuracy
on a chain of $k$ steps under the optimistic assumption of step
independence; in practice, early errors constrain the solution
space for later steps and the compounding is worse. No individual
step fails the five-axis ladder in a way that a walk of that step
alone would catch; each step can be structurally valid, internally
coherent, contextually appropriate, grounded, and environmentally
specified, and the chain still produces a wrong final answer
because an early error propagated. The remedy is not a better
prompt but a better architecture: decompose at checkpoints, verify
intermediate outputs rather than only the final output, and build
observable intermediate states that a human or automated verifier
can inspect. Agent evaluation (Chapter~12) and staged
human-in-the-loop review (Chapter~11) are the primary loci of
intervention.

The sixth is the \emph{distributional mismatch} failure. A model
is shaped not only by its pretraining corpus but by its fine-tuning
and alignment distribution (Chapter~9). The tasks most heavily
represented in the supervised fine-tuning and RLHF phases become
the model's modal behavior; tasks at the periphery of that
distribution receive attenuated, lower-quality performance ---
not because the relevant knowledge is absent from pretraining but
because the gradient signal during fine-tuning shaped behavior
toward the center of the training-task distribution. A model that
achieves high performance on a broad evaluation benchmark may
underperform on the specific domain, register, or procedural
context of a given deployment. The diagnostic signature is a
model that looks capable in evaluation settings and is brittle in
the particular corner of the problem space the deployment
actually requires. This is distinct from drift (the world has not
changed) and from environment failure (the prompt is
well-specified). Distributional mismatch can surface on the
five-axis ladder as a grounding failure: the model produces
confident claims that fail external verification. The ladder
correctly names the symptom; it does not identify the cause as a
training-deployment distribution gap. The distinction matters for
remediation: retrieval and tool use reduce individual grounding
errors but do not address the underlying mismatch. The remedy is
targeted evaluation on representative examples drawn from the
actual deployment distribution, conducted before the model is
relied on in that context rather than after the first production
incident.
...
(Chapter~11) for drift; deliberate review-quality engineering for
complacency; architectural decomposition and staged verification
for compositional failures; deployment-specific evaluation for
distributional mismatch. The ladder below addresses the five-axis
failures --- the class most commonly encountered in practice and
most directly remedied by prompt, system design, and process
changes. Hold the axes loosely: they are not orthogonal, failures
often span multiple, and the ordering is a heuristic, not a
theorem.
