...
\paragraph{The problem.} Chapter~\ref{chap:stat-foundations} told us \emph{what} we want a
...
from Chapter~\ref{chap:stat-foundations}, indexed over the $N_{\mathrm{tok}}$ (context, token) pairs in
...
\end{equation}
where $i$ indexes (context, token) pairs (Chapter~\ref{chap:stat-foundations}, A1.1b) and the
...
layered on top in Chapter~\ref{chap:efficiency} and changes only the per-worker terms, not
the total.
\end{notationbox}
...
reduce $b_{\mathrm o}$; quantization at serving time (Chapter~\ref{chap:adaptation}) reduces
$b_{\mathrm p}$ further. Chapter~\ref{chap:efficiency} revisits the sharded form of
...
\begin{itemize}
  \item emergent capabilities (discussed in Chapter~\ref{chap:pretraining} via
...
what Chapter~\ref{chap:pretraining} calls ``emergent capabilities'' and is \emph{not}
