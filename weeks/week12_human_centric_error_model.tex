...
system-level interventions (Week~9, Week~11) layered on top
...
instances of the same idea. Function-calling APIs (Week~11)
...
trained on cross-entropy (Week~1) optimizes for the
...
(Week~11) extend the same pattern by adding explicit
...
list above. These are, in the language of Week~5, ways of
...
Few-shot prompting (Week~5) is, among other things, an attempt
to encode situational expectations through examples. System prompts
(Week~11) are an attempt to fix the situation up front.
Alignment training (Week~8) shapes the model's defaults along
...
The remedy at the system level is retrieval (Week~9). A
...
through retrieval (Week~9), through tool use, through careful
...
\begin{enumerate}
  \item \emph{Structure}. Does the output parse, compile, validate
        ...
        the remedy lives in Week~5 (decoding) or Week~11
...
\textbf{Structure} is the territory of decoding (Week~5),
constrained sampling, and the structured-output contracts of tool
use (Week~11). The mathematical objects involved are the
...
(Week~1), embeddings (Week~3), and the inference-compute
trade-offs of self-checking and chain-of-thought (Week~5). The
...
(Week~5), prompt construction (Week~9), system prompts
(Week~11), and the tone-and-style components of alignment
(Week~8). The mathematical objects involved are the
preference-model log-likelihood, the few-shot prompt format, and
the RLHF KL penalty that keeps the policy from over-tuning.

\textbf{Grounding} is the territory of retrieval (Week~9), tool
use, the symbol-grounding literature \citep{harnad1990symbol,
bender2020climbing}, and the memorization-versus-hallucination
distinction of Week~5. The mathematical objects involved are
BM25, the dense-retrieval inner product, the ReAct loop, and the
verification predicates of Week~10's hazard analysis.

\textbf{Environment} is the territory of system design rather than
model design. Long-context architectures (Week~4) extend how
much environment can be imported. Agent loops (Week~11)
accumulate environment over time. Documentation, logging, and
incident reporting (Week~10) make the environment the
...
hold. These failures live in Week~8's territory rather than
...
trust boundaries of Week~9 and Week~11 and are addressed by
...
\begin{itemize}
  \item \textbf{Week~1 (Statistical Foundations).} The
  ...
  \item \textbf{Week~5 (Pretraining and In-Context Learning).}
  ...
  \item \textbf{Week~8 (Alignment).} Alignment shapes the
  ...
  \item \textbf{Week~9 (RAG, Tools, Evaluation).} Retrieval is
  ...
  \item \textbf{Week~10 (Governance).} The diagnostic ladder
  ...
  \item \textbf{Week~11 (APIs, Agents, MCP).} Agents accumulate
