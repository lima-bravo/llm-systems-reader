\section{Week 12: A Human-Centric Model for Understanding LLM Errors}
\label{sec:week12-error-model}

\subsection*{Setting the Scene}
\addcontentsline{toc}{subsection}{Setting the Scene}

\paragraph{The problem.}
The eleven chapters of this book have given you the vocabulary of
...
over these objects:
\begin{align}
  \mathrm{structure}(y) &= \mathbb{1}\!\left[y \in \mathcal{V}_{\mathrm{form}}\right] \label{eq:week12-structure}\\
  \mathrm{meaning}(y) &= \mathbb{1}\!\left[y \text{ is internally consistent}\right] \label{eq:week12-meaning}\\
  \mathrm{context}(y; s) &= \mathbb{1}\!\left[y \text{ is appropriate for } s\right] \label{eq:week12-context}\\
  \mathrm{grounding}(y; w) &= \mathbb{1}\!\left[y \text{ is true in } w\right] \label{eq:week12-grounding}\\
  \mathrm{environment}(p; h) &= \mathbb{1}\!\left[p \text{ encodes enough of } h\right] \label{eq:week12-environment}
\end{align}
...
\textbf{(A12.3)} The grounding and environment axes are
fundamentally outside the model's reach: no amount of additional
training, by itself, will let the model verify a claim against the
world or supply context the user did not. These axes are addressed
by \emph{system} mechanisms (Chapter~9) rather than \emph{model}
mechanisms.
\end{notationbox}
...
\subsubsection{Structure: is the output well-formed?}
...
instances of the same idea. Function-calling APIs (Chapter~11)
...
\subsubsection{Meaning: is the output internally coherent?}
...
sequence, a global contradiction. The model has no internal
mechanism that checks the output as a whole against itself;
coherence is a side-effect of training, not an enforced invariant.
...
A model whose meaning is unreliable can be improved by asking it
to check its own work, by using chain-of-thought prompting that
externalizes the reasoning, and by sampling multiple completions
and comparing them. These are, in the language of Chapter~5, ways
of trading inference compute against meaning reliability.

\subsubsection{Context: is the output appropriate for the situation?}
...
Few-shot prompting (Chapter~5) is, among other things, an attempt
to encode situational expectations through examples. System prompts
(Chapter~11) are an attempt to fix the situation up front.
Alignment training (Chapter~8) shapes the model's defaults along
...
\subsubsection{Grounding: does the output connect to reality?}
...
The remedy at the system level is retrieval (Chapter~9). A
...
\subsubsection{Environment: did the prompt encode the situation?}
...
through retrieval (Chapter~9), through tool use, through careful
...
\subsection{The Diagnostic Ladder}
...
\begin{enumerate}
  \item \emph{Structure}. Does the output parse, compile, validate
        ...
        the remedy lives in Chapter~5 (decoding) or Chapter~11
...
A useful heuristic, when the failure feels mixed, is to ask which
axis, if it had been correct, would have made the others
correctable. A grounded answer in the wrong tone can be fixed by
the user; a fluent answer about a fact that does not exist cannot.
That heuristic tends to point at the limiting axis.

\subsection{Connections to Earlier Chapters}
...
\textbf{Structure} is the territory of decoding (Chapter~5),
constrained sampling, and the structured-output contracts of tool
use (Chapter~11). The mathematical objects involved are the
...
(Chapter~1), embeddings (Chapter~3), and the inference-compute
trade-offs of self-checking and chain-of-thought (Chapter~5). The
...
(Chapter~5), prompt construction (Chapter~9), system prompts
(Chapter~11), and the tone-and-style components of alignment
(Chapter~8). The mathematical objects involved are the
preference-model log-likelihood, the few-shot prompt format, and
the RLHF KL penalty that keeps the policy from over-tuning.

\textbf{Grounding} is the territory of retrieval (Chapter~9), tool
use, the symbol-grounding literature \citep{harnad1990symbol,
bender2020climbing}, and the memorization-versus-hallucination
distinction of Chapter~5. The mathematical objects involved are
BM25, the dense-retrieval inner product, the ReAct loop, and the
verification predicates of Chapter~10's hazard analysis.

\textbf{Environment} is the territory of system design rather than
model design. Long-context architectures (Chapter~4) extend how
much environment can be imported. Agent loops (Chapter~11)
accumulate environment over time. Documentation, logging, and
incident reporting (Chapter~10) make the environment the
...
hold. These failures live in Chapter~8's territory rather than
...
trust boundaries of Chapter~9 and Chapter~11 and are addressed by
...
\subsection{Worked Example: Diagnosing a Wrong Code Suggestion}
...
\begin{itemize}
  ...
  \item \textbf{Chapter~3 (Tokenization and Embeddings).} The
        embedding space is the substrate on which meaning is
        encoded; the structure axis sits on top of the
        tokenization pipeline.
  \item \textbf{Chapter~4 (Transformer Architecture).} Context
  ...
  \item \textbf{Chapter~5 (Pretraining and In-Context Learning).}
  ...
  \item \textbf{Chapter~8 (Alignment).} Alignment shapes the
  ...
  \item \textbf{Chapter~9 (RAG, Tools, Evaluation).} Retrieval is
  ...
  \item \textbf{Chapter~10 (Governance).} The diagnostic ladder
  ...
  \item \textbf{Chapter~11 (APIs, Agents, MCP).} Agents accumulate
...
discuss the result with someone who knows the territory. The model
is most useful when it is held loosely.
