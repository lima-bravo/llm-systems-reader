...
extraordinarily capable thing. It is also bounded in two ways that
...
\subsection{Notation and System Assumptions}
...
temperature (Week~1), and $T$ for context length. The generator
...
retrieved chunk set $R_{k_R}(q) = \{c_{(1)}, \dots, c_{(k_R)}\}$
and the user query $q$ into a single token sequence, and
$P_\theta$ is the frozen generator from Week~4. The design space is
...
For a query $q$ with terms $t$ and a chunk $c$ of length $|c|$
tokens drawn from an index with average chunk length $\bar{L}$, the
BM25 score \citep{robertson2009bm25} is
\begin{equation}
  s_{\mathrm{BM25}}(q, c)
  \;=\;
  \sum_{t \in q}
...
trusted and $\mathcal{U} = R_{k_R}(q)$ as untrusted. The core safety
invariant is:
\begin{equation}
...
\begin{figure}[ht]
  ...
  reasoning-trace-based methods in Week~8) but does not cross the
...
positional-encoding behaviour (Week~4) and training-data
...
\subsection*{Derivation Ledger (Ch. 9)}
