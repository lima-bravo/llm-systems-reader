...
a few layers is computationally inert. With it the depth becomes
free in a deep mathematical sense: the cost of computing the
gradient is roughly the cost of computing the forward pass.
...
the question of \emph{which} optimiser was largely settled. The
...
the Chinchilla fit constants of Eq.~\eqref{eq:week2-chinchilla}.
Standing assumptions are:
\textbf{(A2.1)} the SGD-to-SDE approximation
(Eq.~\eqref{eq:week2-langevin}) holds only when $\eta$ is small
...
(Eq.~\eqref{eq:week2-mem-budget}) assumes standard mixed-precision
AdamW with per-worker full optimizer state; ZeRO-style sharding is
layered on top in Week~6 and changes only the per-worker terms, not
the total.
\end{notationbox}
...
Eq.~\eqref{eq:week2-sgd} is approximated by a continuous-time SDE
...
Unroll Eq.~\eqref{eq:week2-adam-m} from $m_0 = 0$:
\begin{align*}
...
\qquad
\text{so}\qquad
\hat m_t \;:=\; \frac{m_t}{1-\beta_1^{\,t}} \;\text{ is unbiased}.
\end{align*}
...
reduce $b_{\mathrm o}$; quantization at serving time (Week~7) reduces
$b_{\mathrm p}$ further. Week~6 revisits the sharded form of
Eq.~\eqref{eq:week2-mem-budget}.
...
\S\ref{sec:week2-notation} the four schedule parameters:
...
\begin{figure}[H]
  \centering
  \input{figures/lr_schedule}
  \caption{Learning rate schedules. Linear warmup to $\eta_{\max}$ at
           step $t_w$, then (teal) cosine decay to a small floor at
           $T$, or (magenta, dotted) inverse-square-root decay
           (Noam). The red dashed constant schedule is shown for
           contrast; it is the naive default that often diverges
...
Eq.~\eqref{eq:week2-gradclip} equals $c / \|g_t\|_2 = 1/5$, and the
clipped gradient is
\[
g_t^{\mathrm{clip}}
  \;=\; g_t \cdot \frac{1}{5}
...
unchanged. This is why the activation rate of Eq.~\eqref{eq:week2-gradclip}
...
Eq.~\eqref{eq:week2-chinchilla-opt} reflects the near-equality of
...
document that uses Eq.~\eqref{eq:week2-chinchilla-opt} to argue for a
capability level, rather than a loss level, is conflating these two
predictive regimes; reject such arguments in design reviews.

\begin{provedassumed}
\emph{Proved (within the fit range).}
Eq.~\eqref{eq:week2-chinchilla} is a regression fit whose exponents
...
\emph{Assumed.}
(i) The parametric form of Eq.~\eqref{eq:week2-chinchilla} — a power
...
what Week~5 calls ``emergent capabilities'' and is \emph{not}
governed by Eq.~\eqref{eq:week2-chinchilla}.
\end{provedassumed}
...
\subsection*{Derivation Ledger (Ch. 2)}
...
\begin{itemize}
  \item \textbf{Empirical-risk objective.}\
        $\mathcal{L}(\theta) = \tfrac{1}{N} \sum_i \ell(\theta; x_i)$
        --- Eq.~\eqref{eq:week2-empirical-loss}.
  \item \textbf{Minibatch SGD update.}\
        Eq.~\eqref{eq:week2-sgd}; assumption (unbiased gradient
