...
redistributes to under-utilized experts. The fixed point is uniform
$P_i = 1/N_e$, and the quadratic-looking form
$\sum_i f_i P_i$ is minimized (at fixed $\sum_i f_i = 1$,
$\sum_i P_i = 1$) exactly when both are uniform --- by rearrangement
of Chebyshev's sum inequality.
\end{derivation}
...
at a fixed active parameter count $N_{\mathrm{active}}$, using the
mixed-precision-AdamW coefficients from
Eq.~\eqref{eq:week2-mem-budget} ($16 N$ bytes for weights + grads +
optimizer, ignoring activations).
\begin{center}
\begin{tabular}{|l|r|r|r|r|}
\hline
Model (training) & $N_{\mathrm{active}}$ & $N_{\mathrm{total}}$ & Train FLOPs/tok & Train memory \\
\hline
Dense 30B           & 30\,B  & 30\,B   & $\approx 6 \cdot 30$\,B  & $16 \cdot 30$\,B $\approx 480$\,GB \\
MoE 8$\times$30B, $k{=}2$ & 30\,B  & 240\,B  & $\approx 6 \cdot 30$\,B  & $16 \cdot 240$\,B $\approx 3.8$\,TB \\
\hline
\end{tabular}
\end{center}
Same per-token compute, $8\times$ the memory. Serving-time
arithmetic is similar: FLOPs/token tracks $N_{\mathrm{active}}$ while
resident memory (weights only, no optimizer state) tracks
$N_{\mathrm{total}}$. This single asymmetry is what forces every
systems choice in this chapter.
