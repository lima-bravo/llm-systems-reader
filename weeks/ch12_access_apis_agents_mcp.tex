...
decode (see Chapter~\ref{chap:efficiency}).
...
\begin{description}
  ...
  \item[A11.3 Trust boundary.] Tool outputs and MCP server
    ...
    retrieved chunks (A9.2, Chapter~\ref{chap:systems}). The host is the security
...
Chapter~\ref{chap:governance}'s governance framing lives here.
...
statistically in Chapter~\ref{chap:pretraining}. A practitioner's quick reference:
...
The KV cache (Chapter~\ref{chap:transformer}, Chapter~\ref{chap:efficiency}) is a per-request optimization. At
...
similarity. This is Chapter~\ref{chap:systems}'s RAG index, used as an agent tool
...
Chapter~\ref{chap:systems} introduced the evaluation stack. The deployment practice
...
Chapter~\ref{chap:systems} introduced the evaluation stack for single-turn and
...
Chapter~\ref{chap:systems} (\S\ref{sec:verification-cost-erosion}) and from
the human-factors angle in Chapter~\ref{chap:error-model}; the complacency effect
...
\textbf{Reward hacking in the agent loop.} Chapter~\ref{chap:alignment}
...
Chapter~\ref{chap:agents} is the outer ring of the reader:

\textbf{Chapter~\ref{chap:transformer}} (transformer architecture) and \textbf{Chapter~\ref{chap:efficiency}}
(efficiency) explain the runtime's job at L7 --- KV cache,
attention, batching --- and why speculative decoding and prompt
caching pay off.

\textbf{Chapter~\ref{chap:pretraining}} (pretraining, in-context learning) motivated the
...
tuned and Chapter~\ref{chap:alignment}'s alignment story shapes what the raw model
under the API actually is.

\textbf{Chapter~\ref{chap:adaptation}} (adaptation, LoRA, quantization) determines what
self-hosted runtimes look like: multi-tenant LoRA swap, NF4
quantization, FP8 inference are all ways to make the L7 layer
commercially viable.

\textbf{Chapter~\ref{chap:alignment}} (alignment) gives you the assistant the API
talks to. Tool-use reliability, refusal behaviour, and
instruction-following accuracy are all downstream of RLHF/DPO
training.

\textbf{Chapter~\ref{chap:systems}} (RAG, tools, evaluation) gave the theory; this
chapter gives the wire format. Retrieval is a tool; evaluation
is a CI stage; the RAG pipeline sits inside the agent loop.

\textbf{Chapter~\ref{chap:governance}} (governance) applies to every layer of the
