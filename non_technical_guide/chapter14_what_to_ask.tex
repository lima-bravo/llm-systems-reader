...
tell the two apart. The groundedness axis of the error model
in Chapter~\ref{chap:error-model} is where hallucinations live.
...
\begin{itemize}
  ...
  \item \emph{What happens when the model is asked about
        ...
        when it lacks groundedness. A poorly calibrated system
...
the attention mechanism (Chapter~\ref{chap:transformer}): the model is sensitive
...
described in Chapter~\ref{chap:transformer} is theoretically capable of relating
...
``lost in the middle'' failure documented in Chapter~\ref{chap:systems}.
...
Chapter~\ref{chap:agents} notes, roughly what SQL injection once was to
...
As Chapter~\ref{chap:governance} describes, the most insidious failure mode of
