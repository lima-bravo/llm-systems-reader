...
token, the vectors we constructed in Chapter~\ref{chap:tokenization} --- and produces
...
described in Chapter~\ref{chap:neural-networks} --- a couple of layers, each computing
...
weighting we described in Chapter~\ref{chap:stat-foundations}.
...
reason the Transformer won. The scaling laws of Chapter~\ref{chap:efficiency} --- the
...
systems (Chapter~\ref{chap:systems}), where a longer context lets the system
...
Chapter~\ref{chap:stat-foundations} as a consequence of the model producing samples; here
...
This chapter built on Chapter~\ref{chap:neural-networks} (the deep stack of layers) and
Chapter~\ref{chap:tokenization} (the embeddings that turn tokens into the vectors the
attention mechanism reads). It is the architectural foundation
for everything that follows: Chapter~\ref{chap:pretraining} trains a Transformer at
scale, Chapter~\ref{chap:efficiency} discusses how big the Transformer can get,
Chapter~\ref{chap:adaptation} modifies a trained Transformer cheaply, and
Chapter~\ref{chap:systems} puts a Transformer on top of a retrieval system.
The lost-in-the-middle effect we have just described will
reappear in Chapter~\ref{chap:systems} as a major constraint on how retrieval
results are arranged in the prompt, and in Chapter~\ref{chap:error-model} as one of
