...
But errors from an early step enter the next step as premises,
and errors compound. An output that is 95\% reliable at each
step falls below 60\% reliability across ten chained steps
--- the errors do not add, they multiply. The remedy is not
to improve each step further but to checkpoint: treat each
intermediate output as a hypothesis to verify before passing
it to the next step.
...
failures (which can appear anywhere). The remedy is
evaluation on the actual deployment population, not only on
benchmarks that may not represent it.
