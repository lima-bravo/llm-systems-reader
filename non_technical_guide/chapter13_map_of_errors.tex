...
The five-axis framework is useful because it is simple. It is
also incomplete. A few classes of failure deserve mention as
cases where the ladder helps less than one might hope.
...
offensive, biased, unethical, in conflict with the values the
user expected the system to hold. These failures live in
...
The second is the \emph{adversarial} failure: an output that
has been induced by a malicious party manipulating one of the
inputs --- the prompt, a retrieved document, a tool result.
The ladder will identify the failure (most often as a context
or grounding failure), but the diagnosis does not by itself
explain that the failure was caused by an attacker. The
adversarial framing is its own discipline, and Chapter~12's
treatment of prompt injection and confused-deputy attacks is
where it belongs.

The third is the \emph{drift} failure: an output that is fine
considered as a single artefact but that, considered as part
of a stream of similar outputs over time, indicates that the
system's behaviour has changed in a way the operator did not
anticipate. Drift is detected statistically rather than
individually, and the monitoring infrastructure of Chapter~11
is what catches it.

The fourth is the \emph{collective-impact} failure: an output
that may be correct for the individual user who received it
but that contributes to a pattern of harm at the population
level. A model that produces correct medical advice for the
average patient may produce systematically worse advice for a
minority subgroup. A model that produces correct recommendations
in aggregate may produce a homogenisation of behaviour that
is harmful in the long run. These failures live in
Chapter~11's territory and are not visible from inspecting
individual outputs.
