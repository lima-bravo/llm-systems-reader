...
\textbf{Groundedness.} Whether the output connects to verifiable
...
that makes no native attempt at groundedness. Retrieval is a
...
wrong tone has failed on groundedness and context simultaneously.
...
them; failures of context tend to drag groundedness. The axes
...
on next-token prediction (Chapter~\ref{chap:stat-foundations}) optimises for the
...
not exist for the model. Few-shot prompting (Chapter~\ref{chap:pretraining}) is,
among other things, an attempt to encode situational
expectations through examples. System prompts (Chapter~\ref{chap:agents})
are an attempt to fix the situation up front. Alignment
training (Chapter~\ref{chap:alignment}) shapes the model's defaults along this
...
\subsection*{Groundedness: does the output connect to reality?}

The fourth axis is whether the output connects to verifiable
reality or has drifted into plausible-sounding fabrication.
The practitioner literature calls a groundedness failure a
...
The groundedness is no longer the model's problem; it is the
retriever's problem and the prompt's problem. Tool use
(Chapter~\ref{chap:systems} and Chapter~\ref{chap:agents}) extends the same idea: a model
...
groundedness and move it outside the language model entirely.

Groundedness failures are compounded by a problem the model
...
In diagnosis, groundedness failures call for a particular
...
Chapter~\ref{chap:alignment}'s territory rather than this one. The five axes
...
harm. Chapter~\ref{chap:governance} is where population-level monitoring belongs.
...
groundedness failure), but the diagnosis does not explain that
...
anything is amiss. Chapter~\ref{chap:agents} is where the discipline of
...
Chapter~\ref{chap:governance} is what catches it.
...
\begin{enumerate}
  \item \textbf{Structure.} Does the output parse, compile,
        ...
        decoding (Chapter~\ref{chap:pretraining}) or the system's constrained
        generation (Chapter~\ref{chap:agents}).
  ...
  \item \textbf{Groundedness.} Are the factual claims in the
...
considers groundedness will deploy systems that are confidently
...
\textbf{Groundedness.} Here the ladder catches something.
...
The final sentence is a different kind of groundedness problem.
...
\medskip
\noindent\textbf{Diagnosis:} one groundedness failure
(wrong ministry for Room 101) and one disguised groundedness
...
concentration is characteristic of groundedness failures. They
...
(Chapter~\ref{chap:pretraining}) and the structured-output contracts of tool use
(Chapter~\ref{chap:agents}). When you see a structural failure, the
...
objective (Chapter~\ref{chap:stat-foundations}), the deep neural network architecture
(Chapter~\ref{chap:neural-networks}), and the inference-time techniques that trade
compute for coherence (Chapter~\ref{chap:pretraining}). When you see a meaning
...
(Chapter~\ref{chap:pretraining}), the attention mechanism (Chapter~\ref{chap:transformer}), the
prompt-construction discipline (Chapter~\ref{chap:systems}), the system
prompts of the API surface (Chapter~\ref{chap:agents}), and the alignment
work that shapes the model's defaults (Chapter~\ref{chap:alignment}). When you
...
\textbf{Groundedness} is the territory of retrieval (Chapter~\ref{chap:systems}),
tool use, the symbol-grounding literature (Harnad, Bender and
Koller), and the memorisation-versus-hallucination distinction
of Chapter~\ref{chap:pretraining}. When you see a groundedness failure, the
...
than model design. Long-context architectures (Chapter~\ref{chap:transformer})
extend how much environment can be imported. Agent loops
(Chapter~\ref{chap:agents}) accumulate environment over time. The
documentation, logging, and incident reporting disciplines of
governance (Chapter~\ref{chap:governance}) make the environment the system sees
...
have --- structure, meaning, context, groundedness, environment;
