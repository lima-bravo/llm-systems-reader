...
\paragraph{The problem.} A computer cannot read words. It can read numbers. The neural
network we described in Chapter~\ref{chap:neural-networks} takes a list of numbers as
...
\paragraph{How we got here.} The earliest computational treatments of language took the
...
network we described in Chapter~\ref{chap:neural-networks}, using the same training
procedure of Chapter~\ref{chap:optimization}. The geometric properties Mikolov
...
\paragraph{Where we stand.} A modern tokenizer is a small, deterministic, frozen artefact.
...
rest of the model --- the deep stack of layers from Chapter~\ref{chap:neural-networks}
plus the attention machinery of Chapter~\ref{chap:transformer} --- operates on these
vectors and produces, at the end, a probability distribution
over which token should come next.

\paragraph{What goes wrong.} Tokenization is a quiet source of several user-visible quirks.
...
see this attack class properly in Chapter~\ref{chap:agents}.
...
is a third. The training procedure of Chapter~\ref{chap:optimization} worked these
...
underpins the retrieval systems of Chapter~\ref{chap:systems}. It is not a
...
operate. We will see in Chapter~\ref{chap:agents} the class of attacks that
...
everything else in the book takes for granted. Chapter~\ref{chap:transformer} takes
the embedded tokens as the input to attention. Chapter~\ref{chap:pretraining}
trains the embedding layer along with the rest of the model and
gives us the larger context of pretraining. Chapter~\ref{chap:systems} uses the
embedding space directly as the basis for retrieval. Chapter~\ref{chap:agents}
