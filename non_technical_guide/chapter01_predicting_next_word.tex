...
another, the elaboration. The Transformer of Chapter~\ref{chap:transformer} is a
particular machinery for assigning those numbers. The training
process of Chapter~\ref{chap:pretraining} is what taught the machinery which numbers
to assign. The alignment work of Chapter~\ref{chap:alignment} is what later
...
Chapter~\ref{chap:error-model} are, almost without exception, traceable to the gap
...
While n-gram researchers were filling hard drives with counts,
a separate tradition was developing in parallel. Researchers
inspired by neuroscience had been experimenting, since the 1950s,
with computing systems built from layers of simple units loosely
modelled on neurons. Each unit receives a list of numbers,
combines them with a set of adjustable weights, and passes the
result to the next layer. The weights are the only thing that
changes during training: show the network an example, measure
how wrong the output was, and nudge the weights slightly in the
direction that would have made the answer more correct. Repeat
millions of times. The weights settle; the network generalises.

The problem was that the earliest versions had only a single
layer --- input connected directly to output --- and a
single-layer network can only detect the simplest patterns.
In 1969, Marvin Minsky and Seymour Papert proved this
limitation formally, and funding for the whole approach largely
collapsed. The next step, adding hidden layers between input
and output, was conceptually straightforward but practically
blocked: there was no efficient method for adjusting the weights
deep inside the network based on how wrong the final answer was.
That problem was solved in 1986, when David Rumelhart, Geoffrey
Hinton, and Ronald Williams showed how to propagate the error
signal backwards through the layers, giving each weight its
share of the blame. Two- and three-layer networks became
trainable. But even those remained too shallow to capture the
long-range structure of language, and for another decade and a
half the architecture continued to underdeliver on its promise.
By the early 2000s the machinery for going deeper was finally
ready.

In 2003, Yoshua Bengio and his coauthors
built a small neural network that learned a continuous
representation of words --- a list of numbers per word, of the
sort we will spend Chapter~\ref{chap:tokenization} understanding --- and used those
...
years old, had not. We will spend Chapter~\ref{chap:tokenization} unpacking what this
...
neural network --- explored in more depth in Chapter~\ref{chap:neural-networks}.
...
describe in Chapter~\ref{chap:alignment} that turned a fluent autocomplete into
...
in Chapter~\ref{chap:pretraining} --- that controls how aggressively it favours the
...
Chapter~\ref{chap:pretraining} under the name \emph{in-context learning}, and in
Chapter~\ref{chap:efficiency} under the name \emph{scaling laws}.
...
\emph{how much} it helps, in a way we will quantify in Chapter~\ref{chap:efficiency}.
...
addressed by changing the data, by retrieval (Chapter~\ref{chap:systems}), or
...
papers we will discuss in Chapter~\ref{chap:efficiency}, beginning with the work
...
something in the original capability. Chapter~\ref{chap:alignment} is devoted to
...
of the technology, and Chapter~\ref{chap:systems} (retrieval) and Chapter~\ref{chap:error-model}
...
of Chapter~\ref{chap:alignment} tends to make the model both more helpful and less
...
choice. We will return to this distinction in Chapter~\ref{chap:error-model} as
...
we will see in Chapter~\ref{chap:transformer} why the architecture makes this not
...
next words reappears in Chapter~\ref{chap:transformer} as the output of the
Transformer, in Chapter~\ref{chap:pretraining} as the basis of pretraining, in
Chapter~\ref{chap:alignment} as the thing alignment modifies, and in Chapter~\ref{chap:error-model}
as the source of the \emph{meaning} and \emph{groundedness} axes
of the error model. The notion that the model is sampling
rather than answering reappears in Chapter~\ref{chap:pretraining} as the temperature
control. The notion that the model is calibrated only weakly
reappears in Chapter~\ref{chap:alignment} as a measurable cost of the alignment
work and in Chapter~\ref{chap:systems} as the reason retrieval, when done well,
helps so much.
