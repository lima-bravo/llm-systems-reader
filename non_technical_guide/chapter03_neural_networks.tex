...
\paragraph{The problem.} We have, by now, a sense of what we want a Large Language Model
to do (Chapter~\ref{chap:stat-foundations}) and a sense of how the model is improved over
time (Chapter~\ref{chap:optimization}). What we have not yet looked at is what is
...
\paragraph{How we got here.} The idea that a network of simple computing units could learn a
...
\paragraph{Where we stand.} A modern neural network used inside a Large Language Model is a
...
The Transformer of Chapter~\ref{chap:transformer} is, in essence, this stack of
...
other inputs in the sequence. We will see in Chapter~\ref{chap:transformer} what
...
\paragraph{What goes wrong.} The failure modes of neural networks are part of the
...
different output. We will see this in Chapter~\ref{chap:transformer} as
\emph{prompt brittleness} and in Chapter~\ref{chap:agents} as
...
a \emph{network}. The training of Chapter~\ref{chap:optimization} adjusts the weights
...
say that the training procedure of Chapter~\ref{chap:optimization} will reliably
...
procedure of Chapter~\ref{chap:optimization} know which of the millions of weights
...
training procedure of Chapter~\ref{chap:optimization}. There is no place inside the
...
Chapter~\ref{chap:systems} --- retrieval, in essence, gives the model the
...
interact. We will see in Chapter~\ref{chap:efficiency} that the relationship
...
retrieval (Chapter~\ref{chap:systems}), or careful prompt engineering at the
...
training procedure of Chapter~\ref{chap:optimization} is adjusting. Every chapter
that follows assumes this picture. Chapter~\ref{chap:tokenization} puts the
\emph{embeddings} that turn words into lists of numbers at the
bottom of the stack. Chapter~\ref{chap:transformer} adds the \emph{attention}
mechanism, the trick that turned ordinary deep networks into
the Transformer. Chapter~\ref{chap:efficiency} returns to the question of how
deep, how wide, and on how much data, framed as a scaling
problem. Chapter~\ref{chap:adaptation} returns to the question of how to modify a
