...
Chapter~\ref{chap:pretraining} trained it to do, which is to produce the most
...
calibration in the error model of Chapter~\ref{chap:error-model}.
...
This chapter built on Chapter~\ref{chap:pretraining} (the base model that alignment
modifies), Chapter~\ref{chap:adaptation} (the LoRA mechanism that makes the
adaptation cheap), and Chapter~\ref{chap:transformer} (the architecture in which
the modifications happen). It is the place where the
calibration property of Chapter~\ref{chap:stat-foundations} comes under pressure, where
the helpfulness behaviour user-visible in Chapter~\ref{chap:agents} is
manufactured, and where the safety properties evaluated in
Chapter~\ref{chap:systems} are produced. Chapter~\ref{chap:systems} builds the systems that
wrap aligned models for deployment. Chapter~\ref{chap:governance} takes up the
governance question of who is responsible for the values
encoded in the alignment, and what it would mean to audit them.
Chapter~\ref{chap:error-model} returns to the failure modes catalogued here as the
