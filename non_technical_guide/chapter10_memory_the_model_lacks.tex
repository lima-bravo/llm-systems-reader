...
described in Chapter~\ref{chap:tokenization}. Once words could be represented as
...
in Chapter~\ref{chap:agents}. Tool use shifted, in about eighteen months,
...
discussed in Chapter~\ref{chap:transformer}. The model reliably attends to the
...
Chapter~\ref{chap:tokenization}. Each document in the corpus is converted, by an
...
We will return to this in Chapter~\ref{chap:agents}.
...
complete. We will return to this in Chapter~\ref{chap:agents}.

\section*{Bridges}
\addcontentsline{toc}{section}{Bridges}

This chapter built on Chapter~\ref{chap:tokenization} (the embedding space that
makes semantic retrieval work), Chapter~\ref{chap:transformer} (the attention
mechanism that determines how the retrieved passages are
processed), Chapter~\ref{chap:pretraining} (the pretrained model that the system
wraps), and Chapter~\ref{chap:alignment} (the aligned model that responds to
user requests). It is the place where most of the
engineering of a deployed LLM product happens, and most of
its surprises. Chapter~\ref{chap:governance} takes up the governance question of
how to assure such a system operationally. Chapter~\ref{chap:agents} looks
at the developer-facing surface of these systems and the
attack patterns that arise. Chapter~\ref{chap:error-model} returns to the
failure modes of retrieval and tool use as the
\emph{groundedness} and \emph{environment} axes of the error
