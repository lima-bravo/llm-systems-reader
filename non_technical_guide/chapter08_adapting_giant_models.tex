...
served by retrieval (Chapter~\ref{chap:systems}) than by adaptation: retrieval
...
infrastructure work of Chapter~\ref{chap:efficiency} is what determines those
numbers.
...
This chapter built on Chapter~\ref{chap:neural-networks} (the deep neural network whose
parameters we are now adapting), Chapter~\ref{chap:pretraining} (the pretraining
procedure that produced the base model), and Chapter~\ref{chap:efficiency} (the
...
turn open base models into products. Chapter~\ref{chap:alignment} describes a
particular kind of adaptation --- the alignment work --- that the
frontier laboratories themselves do to turn a base model into
an assistant. Chapter~\ref{chap:systems} describes an alternative to adaptation
for many use cases, retrieval, which avoids freezing rapidly-
changing knowledge into the model. Chapter~\ref{chap:agents} returns to the
