...
single training run, as we noted in Chapter~\ref{chap:optimization}, takes weeks,
...
The deeper problem is that the scaling laws of Chapter~\ref{chap:optimization} are a
...
again in Chapter~\ref{chap:systems} when we discuss evaluation, and in
Chapter~\ref{chap:error-model} when we discuss the gap between what the model
...
Chapter~\ref{chap:optimization} introduced the Chinchilla scaling law in passing. It
...
distribution. We will return to this in Chapter~\ref{chap:systems} when we
...
This chapter built on Chapter~\ref{chap:optimization} (the scaling-law framing) and
Chapter~\ref{chap:pretraining} (the pretraining recipe) and described the
...
Chapter~\ref{chap:adaptation} looks at the other half of the cost-control story:
how to make a trained model adaptable without retraining it
from scratch. Chapter~\ref{chap:systems} looks at how to make a trained model
useful on data it was not trained on. Chapter~\ref{chap:governance} looks at the
