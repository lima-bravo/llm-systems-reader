...
\paragraph{The problem.} Picture an engineer in 2018, holding a fresh implementation of
...
is the procedure of Chapter~\ref{chap:stat-foundations} (predict the next token) and
Chapter~\ref{chap:optimization} (gradient descent) and Chapter~\ref{chap:transformer} (Transformer),
...
The same model, after the additional steps of Chapter~\ref{chap:alignment}, is
the assistant on the other side of every chat interface you
have used in the last three years.

\paragraph{How we got here.} The connection between language modelling and information
...
the alignment work of Chapter~\ref{chap:alignment} to produce ChatGPT.
...
Transformer of Chapter~\ref{chap:transformer} trained by the procedure described in
this chapter.

\paragraph{Where we stand.} A modern pretraining run has the following character. The data
...
The model is a Transformer of the kind described in Chapter~\ref{chap:transformer},
sized according to the Chinchilla scaling law of Chapter~\ref{chap:optimization} to
...
descent of Chapter~\ref{chap:optimization} with the cross-entropy loss of Chapter~\ref{chap:stat-foundations}.
...
subject of Chapter~\ref{chap:alignment}. The base model is the raw material; the
...
\paragraph{What goes wrong.} Pretraining on internet-scale text has costs that the loss
...
its cut-off. We will see in Chapter~\ref{chap:systems} how retrieval can fill
...
\begin{enumerate}
  ...
  \item \textbf{Preference tuning.} The instruction-tuned model
        ...
        RLHF, DPO, and Constitutional AI work of Chapter~\ref{chap:alignment}.
\end{enumerate}
...
stages in Chapter~\ref{chap:alignment}. The rest of this chapter is about
pretraining itself.
...
We met the sampling step briefly in Chapter~\ref{chap:stat-foundations}. It is worth
...
later interventions (Chapter~\ref{chap:alignment}), or external knowledge sources
(Chapter~\ref{chap:systems}).
...
system (Chapter~\ref{chap:systems}) that supplies post-cut-off information.
...
\paragraph{Emergence claims need scrutiny.} In 2022, Jason Wei
and his coauthors documented a striking pattern: certain
capabilities --- multi-step arithmetic, chain-of-thought
reasoning, code synthesis --- appeared to appear \emph{suddenly}
as model scale crossed a threshold, rather than improving
gradually. They named the phenomenon \emph{emergent abilities}.
A 2023 paper by Rylan Schaeffer, Brando Miranda, and Sanmi
Koyejo offered a more sceptical account: when those same
capabilities are measured with a continuous metric rather than
a pass-or-fail score, the discontinuity largely disappears.
The apparent jump, on their account, is mostly a property of
the measuring instrument, not of the underlying model. The
Schaeffer critique has substantially shifted the field toward
the smooth-scaling view. Most capabilities that once looked
emergent now look, on closer measurement, like ordinary
gradual improvement. A minority of cases still resist a clean
smooth account. The practical implication is the same in
either case: do not extrapolate confidently from the current
generation of models to the next on the basis of what did or
did not appear to emerge in the generation before it.

\section*{Bridges}
\addcontentsline{toc}{section}{Bridges}

This chapter built on Chapter~\ref{chap:stat-foundations} (the cross-entropy objective),
Chapter~\ref{chap:optimization} (the optimisation procedure), Chapter~\ref{chap:neural-networks} (the deep
neural network), Chapter~\ref{chap:tokenization} (the tokenizer and embeddings), and
Chapter~\ref{chap:transformer} (the Transformer architecture). It described what
gets done with all of those components when they are combined
at scale. Everything that follows assumes a pretrained base
model: Chapter~\ref{chap:efficiency} discusses what to do when the budget for
pretraining is binding, Chapter~\ref{chap:adaptation} discusses how to adapt a
trained base model cheaply, Chapter~\ref{chap:alignment} turns the base model into
an assistant, and Chapter~\ref{chap:systems} wraps the assistant in retrieval
