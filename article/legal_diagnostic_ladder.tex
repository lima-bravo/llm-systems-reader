...
diagnosable when a specific draft is in front of you. Four
...
practice area --- specialty courts, narrow regulatory domains,
emerging statutory areas --- the root cause may be that the
tool's fine-tuning distribution is misaligned with that niche.
Retrieval and citation verification address individual errors but
not the underlying pattern. The systemic remedy is practice-area
evaluation on representative matters before deploying the tool in
that context, not per-matter augmentation.
...
These four failure classes require different responses than the
...
culture for complacency. The ladder below does not address them.
It is a procedure for the five-rung failures --- the class that
shows up most often in practice and that yields most directly to
prompt discipline, retrieval, and reviewer skill.
...
For multi-step doctrinal analysis --- determining standing, then
jurisdiction, then the applicable standard, then element
satisfaction --- apply the ladder at each material intermediate
step, not only at the final conclusion. A chain where every
individual step passes the five-rung walk is not therefore sound;
a subtle mischaracterization of the standard in an early step
propagates through the downstream reasoning without any single
link appearing defective.
