...
\section*{What the ladder does not capture}

The five-rung framework is useful because it is simple. It is also
incomplete. Six classes of failure deserve explicit mention
because they are directly relevant to legal practice and escape
the ladder entirely.
...
fixed at its training cutoff. The walking example above already
surfaces this: one of the cases ``has been distinguished in a
more recent appellate decision'' --- which is information the
model could not have had if that decision postdates its training.
Retrieval-augmented tools partially address this by fetching
current authority, but only for the specific queries they run;
the model's background assumptions about doctrinal landscape
remain frozen. For rapidly evolving areas --- regulatory
rulemaking, recent circuit splits, new statutory amendments ---
the gap between what the model ``knows'' and what the law
currently says is not a grounding failure in the fabrication
sense. It is a staleness failure that passes every rung of the
ladder until someone with current doctrinal knowledge reads the
output. Currency checking, in areas where it matters, remains
...
or jurisdiction of a given matter. The remedy is targeted
evaluation on representative examples from the specific practice
area before deploying the tool in that context, rather than
relying on general benchmark performance.
